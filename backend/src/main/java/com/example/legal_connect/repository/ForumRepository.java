...
public interface ForumRepository extends JpaRepository<Post, Long> {
    ...
    /**
     * Find reported posts by search term (title or content) with status
     */
    default Page<Post> findReportedPostsBySearchTermAndStatus(int reportCount, Boolean isActive, String searchTerm, Pageable pageable) {
        return findByReportCountGreaterThanAndIsActiveAndTitleContainingIgnoreCaseOrContentContainingIgnoreCase(
            reportCount, isActive, searchTerm, searchTerm, pageable);
    }
    
    /**
     * Find reported posts by search term (title or content)
     */
    default Page<Post> findReportedPostsBySearchTerm(int reportCount, String searchTerm, Pageable pageable) {
        return findByReportCountGreaterThanAndTitleContainingIgnoreCaseOrContentContainingIgnoreCase(
            reportCount, searchTerm, searchTerm, pageable);
    }
}